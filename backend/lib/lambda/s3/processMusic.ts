...
const lambdaEventSchema = SqsSchema;
 ...
 * @returns
 */
const lambdaHandler = async (event: SafeParsedEvent, context: Context) => {
  logger.addContext(context);
  logger.logEventIfEnabled(event);

  if (!event.success) {
    // ここで event.error がある場合、SQS イベント自体が構造として不正
    // API Gateway側でValidationを通しているはずなので、通常はここに来ることはないはず
    logger.error("Invalid SQS event", { error: event.error });
    return;
  }
  ...
  for (const record of event.data.Records) {
    ...
    if (!parsed.success) {
      ...
      continue; // do not retry invalid messages
    }
    ...
    try {
      ...
      // Throw on processing errors so SQS can retry / DLQ
