...
抽象化はしない
理由としては、`外部サービスではなく`、あくまでも標準のJavaScriptのHTMLMediaElementを扱いやすくしたライブラリだからである
抽象化したいのであれば、最初からhls.jsを使用せず、`HTMLMediaElementを直接利用すべき` である
再生ロジックだけ関数等で分離して定義してあればOK

### S3/DynamoDB

保存処理等はrepositoryとして抽象化する
外部サービスだから
AppSyncからAPI Gatewayに移行したが、抽象化しているとView側のコードは変更不要だったりして、影響範囲が分かりやすく、楽になる

## Lighthouse

![lighthouse](./docs/lighthouse.png)

- chrome dev tool MCPを利用してパフォーマンス改善するとよい
